...
describe('titles/[slug]/edit +layout load', () => {
  it('gates the route through requireCapability with catalog.edit', async () => {
    ...
    // The PageLoad/LayoutLoad event has many fields the helper doesn't use;
    // cast through unknown rather than stubbing the whole SvelteKit shape.
    await load({ fetch: fetchStub, url } as unknown as Parameters<typeof load>[0]);
