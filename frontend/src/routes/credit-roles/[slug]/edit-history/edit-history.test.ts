...
describe('credit-role edit-history SSR route', () => {
	it('loads edit history from the backend API', async () => {
		...
		const result = await load({
			fetch,
			url: new URL('http://localhost:5173/credit-roles/test-role/edit-history'),
			params: { slug: 'test-role' }
		...
		expect(request.url).toBe('http://localhost:5173/api/pages/edit-history/credit-role/test-role/');
	});
