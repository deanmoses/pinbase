<script lang="ts">
	...
	import {
		type SeriesSaveResult,
		saveSeriesClaims,
		type FieldErrors,
		type SaveMeta
	} from './save-series-claims';
	...
	let {
		...
		onerror,
		ondirtychange = () => {}
	}: SectionEditorProps<SeriesEditView> = $props();

	function extractFields(series: SeriesEditView): NameFields {
		return {
			name: series.name,
			slug: series.slug
		};
	...
	export async function save(meta?: SaveMeta): Promise<void> {
		...
		const result: SeriesSaveResult = await saveSeriesClaims(slug, {
