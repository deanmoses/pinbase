...
def _parse_month(s: str | None) -> int | None:
    ...
    # Prefix match handles typos like "Aprili" (starts with "april").
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        credit_queue: list[tuple[int, str, str]] = []  # (model_pk, name, role)
        matched = 0
        skipped = 0
        ...
        for row in rows:
            ...
            matched += 1
            self._collect_claims(pm, row, pending_claims, credit_queue)
    ...
    def _collect_claims(
        ...
        # Design credits from Heading/Info column pairs.
        for i in ("1", "2", "3"):
