...
- **DONE — Status-tuple 422 / 200 bodies now construct the declared Schema.** `entity_crud._delete` / `_restore` / `_delete_preview` (taxonomy registrars), `machine_models.delete_model` / `restore_model`, `titles.delete_title` / `restore_title`, `people.delete_person` / `restore_person` all flipped. Return types narrowed from `dict[str, Any] | Status[dict[str, Any]]` to `<SuccessSchema> | Status[<SoftDeleteBlockedSchema | AlreadyDeletedSchema>]` (or `Status[ErrorDetailSchema]` on restore). `_delete_preview` also flipped from `dict[str, Any]` to `TaxonomyDeletePreviewSchema` while the file was open — same idiom, out-of-scope bonus. Structural dispatch preserved: construction-time schema choice matches the prior dict-time dispatch (which is what Step 3.2 validated). No baseline entries for any of this — pure drift cleanup.
- **`locations.py` traffics in nested `dict[str, Any]` trees.** `_get_location_detail`, `_ancestors_of`, `_get_manufacturers_for_pks` all return nested dicts, and the four `get_location_{1..4}` endpoints + `list_locations` return `dict[str, Any]` through to Ninja. `LocationIndexSchema` / `LocationDetailSchema` already exist. Heavier than the Step 7.1 items because the helpers are recursive-tree-shaped — plan it as its own bullet, not residual cleanup.

  **Design notes for the next session** (surfaced on a survey pass but not implemented — fresh eyes wanted):
  - **Internal vs wire shape.** `_get_location_tree`'s dict entries carry `manufacturer_pks: frozenset[int]` alongside the user-visible fields. That frozenset is internal plumbing — it's how `_get_location_detail` resolves `manufacturers` via `_get_manufacturers_for_pks(node["manufacturer_pks"])`, and it must not leak to the wire. A small frozen dataclass (`_LocationNode`) for the internal shape, then projecting to `LocationIndexCountry` / `LocationChildRef` / `LocationDetailSchema` at the endpoint boundary, is the natural split. The two types are not the same shape — don't try to unify.
  - **Cache contract is load-bearing.** `_get_location_tree()` pickles its return into the Django cache under `LOCATIONS_TREE_KEY` (timeout=None, invalidated by `invalidate_all()`). Switching from `tuple[dict[str, dict[str, Any]], dict[str | None, list[str]]]` to a dataclass-based shape changes what gets pickled. Either (a) bump the cache key so existing entries miss cleanly after deploy, or (b) verify the dataclass pickle round-trips and plan for a stale cache at cutover. Don't assume; the cache survives restarts.
  - **Endpoint layer is trivial once helpers are flipped.** `list_locations` + `get_location_{1..4}` are one-liners over `_get_location_detail` / the tree builder. They'll fall out once `_get_location_detail -> LocationDetailSchema` and `list_locations -> LocationIndexSchema`. Don't start there.
  - **`_get_manufacturers_for_pks` is a separate conversion.** Its return `list[dict[str, Any]]` maps cleanly to `list[LocationManufacturerSchema]` — no internal/wire split like the tree helpers. Same pattern as the Step 7.1 easy batch.
  - **Scope check.** Baseline-count impact is ~zero (locations.py already passes `disallow_untyped_defs`); this is drift cleanup that removes the largest remaining `dict[str, Any]` cluster in `catalog/api`. If the cache-contract question turns into a bigger discussion, split it off and tackle just the endpoint + `_get_manufacturers_for_pks` side first.
