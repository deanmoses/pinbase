...
| Entry point                                                                                                                                                            | Writes claims?                      | Writes resolved tables?              | Writes materialized relationships? | Validation scope                                                                                                                                                 | Bypasses save/full_clean?                                  | Post-hoc audit only?                                          |
| ---------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------- | ------------------------------------ | ---------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------- | ------------------------------------------------------------- |
| **All ingest commands** → `bulk_assert_claims()` (`ingest_ipdb`, `ingest_opdb`, `ingest_pinbase`, `ingest_wikidata`, `ingest_wikidata_manufacturers`, `ingest_fandom`) | Yes — `bulk_create()` of Claim rows | No — separate resolve call follows   | No — separate resolve call follows | **Mojibake only** — `validate_no_mojibake()` on string claim values; no field range validators, no markdown cross-reference, no FK existence, no cycle detection | Yes — `bulk_create()` bypasses `save()` and `full_clean()` | No — claims written first; resolve is called explicitly after |
...
| Entry point                                                                                                         | Writes claims?                                    | Writes resolved tables?                                             | Writes materialized relationships?              | Validation scope                                                                                                               | Bypasses save/full_clean?                                  | Post-hoc audit only?                      |
| ------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------- | ------------------------------------------------------------------- | ----------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------ | ---------------------------------------------------------- | ----------------------------------------- |
| **`ingest_pinbase`** — `series.titles.add(*titles)` (line 1560)                                                     | **No**                                            | No                                                                  | **Yes — directly writes series-title M2M rows** | **None**                                                                                                                       | Yes — `.add()` bypasses `save()` and `full_clean()`        | **N/A — no claim ever created**           |
...
| **`ingest_pinbase`** — `Title.objects.filter().update(slug=...)` / `.update(fandom_page_id=...)` (lines 1547, 1551) | **No**                                            | **Yes — writes `slug` and `fandom_page_id` directly to Title**      | No                                              | **None** — `QuerySet.update()` bypasses all validation                                                                         | Yes — `QuerySet.update()` bypasses `save()` entirely       | **N/A — no claim ever created**           |
| **`ingest_pinbase`** — `MachineModel.objects.bulk_update([...], ["created_at", "model_slug"])` (line 1683)          | **No**                                            | **Yes — writes `created_at` and `model_slug` to MachineModel**      | No                                              | **None**                                                                                                                       | Yes                                                        | **N/A — no claim ever created**           |
...
| `opdb_id`, `fandom_page_id`, `needs_review`, `needs_review_notes` | `Title`                   | `title.py:25-32`      | `ingest_pinbase` → `bulk_update(["fandom_page_id"])`          |
...
| -------------------- | -------------- | -------------------------------------------------------- |
| `opdb_id`, `ipdb_id` | `MachineModel` | `ingest_pinbase` → `bulk_update(["opdb_id", "ipdb_id"])` |
...
| ----------------------- | ----------------------------------------------- | ------------------------------------------------------------------------------------------------------------------ |
| Series-title membership | `ingest_pinbase` → `series.titles.add(*titles)` | **Must become claim-controlled** — which titles belong to a series is an editorial decision requiring audit trail. |
