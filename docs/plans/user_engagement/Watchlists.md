...
How this project should think about watchlists as a product, based on how comparable systems use them.

## Background and Business Rationale

This project's engagement model is built around stewardship rather than ownership. The goal is not to give contributors exclusive control over pages. The goal is to make it easy for the people who care most about a page to notice changes, inspect them, and correct them when needed.

That matters because this project is explicitly trying to run an open, community-corrected knowledge base without a staffed moderation queue. In that model, watchlists are not a peripheral convenience feature. They are one of the mechanisms that make the quality model viable. Edit history makes bad edits reversible. Recent changes makes site-wide activity visible. Watchlists make that visibility personal and actionable.
...
This is why this project should not think about watchlists primarily as a bookmarking tool. A bookmark helps a user return to something later. A watchlist helps a user take responsibility for something over time. Those can overlap, but the product intent is different.
...
MediaWiki is the clearest reference point because its watchlist is part of the same basic governance model this project is aiming for: open editing, visible history, and community correction rather than pre-publication review.
...
This is the strongest conceptual match for this project.
...
The direct analogy for this project is that a watchlist should not only collect pages. It should help contributors answer questions like: "What changed on things I care about?" and "Why am I seeing this item?" If this project only builds the watch action and ignores the triage layer, the feature will become noisy as activity grows.
...
That is the pattern this project should learn from.

## Theory for Evaluating Watchlist Features

This project should evaluate watchlist features with a simple question:
...
This project should assume that every convenience feature that adds watches must be paired with a way to clean up the result.
...
### 5. Does it fit this project's actual scale?

This project should resist borrowing heavy watchlist machinery from large enterprise tools unless the underlying problem is real. Early on, a clean watch model tied to recent changes will likely matter more than advanced routing, complex watch taxonomies, or elaborate subscription hierarchies.
...
Features that only improve private organization may still be useful, but features that reinforce active stewardship have more strategic value for this project.

## Features

The right feature set is the one that best supports the theory above, not the longest list. This project should think in terms of a compact core, then selective expansion.
...
- **Auto-watch on edit.** This is a strong fit for this project's stewardship model because editing is the clearest signal that a user may want to keep an eye on future changes. It should be a user preference, not an irreversible rule.
- **Clear indication of why an item is watched.** If this project supports both explicit watches and auto-watches, users should be able to tell the difference. This helps trust and makes management easier.

### Features to consider later, if the need appears

- **Broader watch scopes.** Comparable systems often support watching collections, spaces, or categories. This project should only add broader scopes when contributors actually think in those groupings and when the narrower per-record model starts to feel insufficient.
...
- **Complex subscription hierarchies too early.** The more layers of watching, following, muting, and inheriting this project adds, the harder the system becomes to understand. The product should stay legible.

## Working Position

This project should treat watchlists as stewardship infrastructure.
...
That set is small, but it serves the real business case. It helps this project sustain quality without staff queues, supports the contributor identity the site depends on, and leaves room to expand only when real usage justifies it.
