...
Anonymous visitors are not assigned a pseudonym. PostHog runs with `persistence: "memory"`, so each page load is a fresh anonymous distinct_id — this matches the "not linked across sessions" constraint in [Analytics.md](Analytics.md#identifiability).
